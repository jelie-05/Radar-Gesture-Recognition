...
class Dinosaur:
    ...
    def update(self, userInput):
        ...
        if userInput=='pull' and not self.dino_duck:
            self.dino_duck = False
            self.dino_run = False
            self.dino_jump = True
        elif userInput=='push' and not self.dino_jump:
            self.dino_duck = True
            self.dino_run = False
            self.dino_jump = False
        elif not (self.dino_jump or self.dino_duck or userInput=='pull'):
            self.dino_duck = False
            self.dino_run = True
            self.dino_jump = False
...
class PyGameInference:
    def __init__(self, observation_length, num_classes):
        ...
        self.cooldown_time = 0.05  # seconds
        ...
        with Device() as device:
            ...
            while not self.stop_event.is_set():
                ...
                if rdtm_np.shape[3] >= self.observation_length:
                    ...
                    #     self.label = self.get_class_name(max_idx)
                    ...
                    if time.time() - self.last_print_time > self.cooldown_time:
                        ...
                        with self.label_lock:
                            self.label = self.get_class_name(max_idx)
                        print(f"Detected: {self.label} | Prob: {prediction.max() * 100:.2f}%")
                        if self.label == 'pull' or self.label == 'push':
                            self.countdown_time = 0.3
                        else:
                            self.countdown_time = 0.1
...
def menu(death_count):
    ...
    # Start inference.run() ONCE if not already running
